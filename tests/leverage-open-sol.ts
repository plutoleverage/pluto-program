...
describe("leverage-open-sol", () => {
  ...
  it("Is initialized!", async () => {
    try {
      ...
      const amountToSwap = ((1 * leverage_fee) + (2 * borrow_fee)) * 1e8 // 1 + 2 USDC (6 decimals)
      // Compute routes
      ...
      const wrap_sol_ix = await program.methods.wrapSol(new anchor.BN(1*1e8)).instruction();
      ...
      }, new anchor.BN(1 * 1e8), 3000).accounts({
        vault: vaultAccount,
      }).instruction();
      ...
      let instructions = [
        ...
        wrap_sol_ix,
        fund_ix,
        ...setupInstructions.map(instructionDataToTransactionInstruction).filter((instruction) => {
          return instruction.programId.toString() != '11111111111111111111111111111111';
        }),
