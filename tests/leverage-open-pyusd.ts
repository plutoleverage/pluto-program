...
describe("leverage-open-pyusd", () => {
  ...
  const nativeCollateralPriceOracleamountToSwap = new PublicKey(accounts.nativeCollateralPriceOracle);
  ...
  it("Is initialized!", async () => {
    try {
      ...
      const amountToSwap = ((1 * leverage_fee) + (10 * borrow_fee)) * 1e6 // 1 + 2 USDC (6 decimals)
      // Compute routes
      ...
      }, new anchor.BN(1 * 1e6), 3000).accounts({
        vault: vaultAccount,
      }).instruction();
